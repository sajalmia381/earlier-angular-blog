...
import { PostAdd, PostListFail, PostListRequestAction, PostListSucessAction, PostUpdate } from "app/actions/postAction";
...
export class PostsStateService {
    ...
    getPostList = (force = false): [Observable<Post[]>, Observable<Boolean>, Observable<Boolean>] => {
        ...
        combineLatest([postLoaded$, postLoading$])
            .pipe(take(1))
            .subscribe(data => {
    ...
    addPost = (formData: { title?: string; body?: string }) => {
        // request api => created Post
        // Alternative post create
        // make custom post
        this.store.dispatch(new PostAdd({ data: { ...formData } }));
    };

    updatePost = (data) => {
        // request api => Response Updated
        this.store.dispatch(new PostUpdate({ data }));
    };
}
